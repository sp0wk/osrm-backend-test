...
namespace
{
std::string getWrongOptionHelp(const engine::api::RouteInspectionParameters &parameters)
{
    std::string help;
    ...
    {
        help = "Number of coordinates must be exactly two.";
    }
...
engine::Status RouteInspectionService::RunQuery(std::size_t prefix_length,
                                                std::string &query,
                                                osrm::engine::api::ResultT &result)
{
    ...
    auto parameters =
        api::parseParameters<engine::api::RouteInspectionParameters>(query_iterator, query.end());
    ...
    return BaseService::routing_machine.RouteInspection(*parameters, result);
}
} // namespace osrm::server::service
